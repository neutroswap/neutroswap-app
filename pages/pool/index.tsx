                      ...
                      <div className="space-y-2">
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex items-center space-x-2">
                            <span className="text-left font-medium text-lg">{position.token0.symbol}</span>
                            <span className="text-left font-medium text-lg opacity-25">/</span>
                            <span className="text-left font-medium text-lg">{position.token1.symbol}</span>
                          </div>
                        </div>
                        <div className="flex text-sm space-x-4">
                          <div className="flex space-x-2">
                            <span className="opacity-50">Pool share</span>
                            <span className="font-semibold">{decimalFormat(Number(formatEther(position.poolShare)) * 100, 2)}%</span>
                          </div>
                          <div className="flex space-x-2">
                            <span className="opacity-50">Owned LP token</span>
                            <span className="font-semibold">{decimalFormat(Number(formatEther(position.userBalance)), 2)} {position.symbol}</span>
                          </div>
    ...
    <div>
      ...
      {existingPool === '0x0000000000000000000000000000000000000000' && (
        <div>
          ...
          <p className="max-w-3xl mx-auto text-sm text-neutral-400 dark:text-neutral-600 text-center">No pool found! But, you can create it now and start providing liquidity.</p>
        </div>
      )}
    </div>
  )
