  ...
  if (error) {
    ...
    for (let i = 0; i < 4; i++) {
      let vault: Vault = {
        pid: i,
        lockup: _whichLock(i),
        tokenLogo: "https://iixqzxsufrbufhtizuuk.supabase.co/storage/v1/object/public/asset/neutro_logo.svg",
...
export async function composeData(vaults: Vault[] | null): Promise<Vaults | null> {
  ...
  for (const vault of vaults) {
    const rpsResult = contractCalls.results[call1].callsReturnContext.filter(res => res.methodParameters[0] === vault.pid);
    const stakedResult = contractCalls.results[call2].callsReturnContext.filter(res => res.methodParameters[0] === vault.pid);
    ...
    if (rpsResult) {
      ...
      vault.valueOfVault = (vault.tokenPrice * parseFloat(vault.totalStaked)).toFixed(2).toString()
      vault.details = {
