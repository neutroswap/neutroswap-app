...
export async function composeData(
  ...
  for (const farm of farms) {
    ...
    if (result) {
      ...
      farm.details = {
        totalStaked: formatEther(BigInt(totalStaked.hex)),
        totalStakedInUsd: "",
        pendingTokens: formatEther(BigInt(pendingTokens[0].hex)),
        pendingTokensInUsd: "",
      };
    }
...
export async function totalValueOfLiquidity(farmHoldings: FarmHoldings) {
  ...
  // get all pids
  const contractCallContext: ContractCallContext[] = farmHoldings.farms.map(
    (farm) => ({
  ...
  for (const farm of farmHoldings.farms) {
    ...
    farm.valueOfLiquidity = (
      ...
      .toString();
    farm.lpPrice = (
      parseFloat(farm.valueOfLiquidity.toString()) /
      parseFloat(farm.circulatingSupply.toString())
    )
      .toFixed(2)
      .toString();

    if (!farm.details?.totalStaked) {
      throw Error;
    }
    farm.details = {
      totalStaked: farm.details.totalStaked,
      totalStakedInUsd: (
        parseFloat(farm.details.totalStaked) * parseFloat(farm.lpPrice)
      ).toString(),
      pendingTokens: farm.details.pendingTokens,
      pendingTokensInUsd: (parseFloat(farm.details.pendingTokens) * neutroPrice)
        .toFixed(2)
        .toString(),
    };
